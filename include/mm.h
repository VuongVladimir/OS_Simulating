...
#define PAGING_PAGE_PRESENT(pte) (pte & PAGING_PTE_PRESENT_MASK) // IMPORTANT
...
#define PAGING_SWP(pte) ((pte & PAGING_SWP_MASK) >> PAGING_SWPFPN_OFFSET) // IMPORTANT
...
#define PAGING_OFFST(x) GETVAL(x, PAGING_OFFST_MASK, PAGING_ADDR_OFFST_LOBIT) // IMPORTANT
/* Extract Page Number*/
#define PAGING_PGN(x) GETVAL(x, PAGING_PGN_MASK, PAGING_ADDR_PGN_LOBIT) // IMPORTANT
/* Extract FramePHY Number*/
// #define PAGING_FPN(x) GETVAL(x, PAGING_FPN_MASK, PAGING_ADDR_FPN_LOBIT) // IMPORTANT
#define PAGING_FPN(pte) (pte & ((1 << 13) - 1))
/* Extract SWAPFPN */
...
// Moi add vao
/* Define SWPTYPE in the PTE */
#define PAGING_SWPTYP_BITS 3                                                 // Number of bits you want to allocate for SWPTYPE
#define PAGING_SWPTYP_LOBIT (PAGING_PTE_SWPTYP_LOBIT)                        // Starting bit, assuming it starts at bit 0
#define PAGING_SWPTYP_HIBIT (PAGING_SWPTYP_LOBIT + PAGING_SWPTYP_BITS - 1)   // Ending bit
#define PAGING_SWPTYP_MASK GENMASK(PAGING_SWPTYP_HIBIT, PAGING_SWPTYP_LOBIT) // Mask for SWPTYPE bits

// #define SET_SWPTYP(pte, value) SETVAL(pte, value, PAGING_SWPTYP_MASK, PAGING_SWPTYP_LOBIT)  // Set SWPTYPE value in PTE
#define PAGING_SWPTYP(pte) GETVAL(pte, PAGING_SWPTYP_MASK, PAGING_SWPTYP_LOBIT) // Get SWPTYPE value from PTE
// cai tren moi add vao
